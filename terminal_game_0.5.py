...
game_maps = {  # # | - _ ¯ ♦ ■ §
    ...
    1: {  # Episode 1 Başlangıç: YER ÜSTÜ BARIŞI
    ...
    2: {  # Episode 2 Gelişme: PARÇALANMA
        "name": "Harita2", "height": 15, "width": 25,
        "map": [["X", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " "],
                ["_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", " ", " ", " ", " ", " ", " "],
                [" ", " ", " ", " ", " ", "|", " ", " ", " ", " ", " ", " ", "|", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " "],
                [" ", " ", " ", " ", " ", "|", " ", " ", " ", " ", " ", " ", "|", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " "],
...
def check_position():
    ...
    # INDIGO TOWN
    if episode == 1:
        if (x_pos, y_pos) == (18, 13) or (5, 11) or (4, 9) or (3, 13):
            current_state = State.DIALOGUE
        else:
            current_state = State.PLAYING
        return current_state
    ...
    while True:
        ...
        if current_state == State.PLAYING and episode == 0:
            ...
            episode, x_pos, y_pos, step = move_player(width, height, game_maps, episode, move_input=choice,
                                                      x=x_pos, y=y_pos)
            game_maps[episode]["map"][y_pos][x_pos] = change_cursor(choice)
            current_state = check_position()

        # Portal (Break)
        if current_state == State.DIALOGUE and episode == 0 and (x_pos, y_pos) == (4, 3):
            speak("Where are you heading to?")
            speak(">1) INDIGO TOWN")
            speak(">2) BALDURS GATE")
            speak(">3) BIKINI BOTTOM TOWN")
            choice = input(">")
            if choice == "1":
                terminal_cleaner()
                print("------------------------")
...
def episode_1():
    ...
    while True:
        ...
        # ANANAS EV
        if current_state == State.DIALOGUE and episode == 1 and (x_pos, y_pos) == (4, 9) and not pineapple_home_dialogue_done:
        ...
        # SUNGER BOB
        if current_state == State.DIALOGUE and episode == 1 and (x_pos, y_pos) == (3,13) and not sponge_bob_dialogue_done:
                ...
                if choice.isdigit():
                    ...
                    if 1 <= choice <= len(sponge_bob_dialogues):
                        ...
                        if answer == "Selam!":
                            ...
                            speak("O denizanalarını toplamaya gideceğim. \n Eğer yine de bir özür dilemek istersen, bana yardıma gelmeye ne dersin??")
                            time.sleep(1)
                            speak(">1) Harika! Sana yardım etmek isterim.")
                            speak(">2) Bunu yapmak istediğimden emin değilim.")
                            choice = input(">")
        ...
        # PATRİCK
        if current_state == State.DIALOGUE and episode == 1 and (x_pos, y_pos) == (18, 13) and not patrick_dialogue_done:
            speak( "**Bikini Bottom'un kumlu plajında dolaşırken bir horlama sesi duyuyorsun. "
        ...
        # GARRY
        if current_state == State.DIALOGUE and episode == 1 and (x_pos, y_pos) == (5, 11) and not garry_dialogue_done:
        ...
        # DENİZANALARI
        if current_state == State.PLAYING and episode == 1 and (x_pos, y_pos) == (24, 6) and not jellyfish_dialogue_done:
            speak("Bikini Bottom'un eşsiz ve renkli deniz yaşamının önemli bir parçası olan denizanaları burada toplanmış gibiler.")
        ...
        # SQUİDWARD
        if current_state == State.PLAYING and episode == 1 and (x_pos, y_pos) == (11, 9) and not squidward_dialogue_done:
            speak("Squidward' un evinin önüne geldin.")
            speak("Klarnetinin sesini duyuyorsun.")
            squidward_dialogue_done = True

        # KRUSTY KRAB
        if current_state == State.PLAYING and episode == 1 and (x_pos, y_pos) == (5, 6) and not krusty_krab_dialogue_done:
            speak("Bay Yengecin ünlü restoranı Krusty Krab'ın önünde duruyorsun.")
